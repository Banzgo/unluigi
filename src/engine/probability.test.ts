...
import { calculateStatistics, getProbabilityAtLeast, getProbabilityExact } from "./probability";
...
describe("calculateStatistics", () => {
	...
	it("should build probability distribution", () => {
		...
		const oneWound = results.probabilityDistribution.find((p) => p.wounds === 1);
		expect(oneWound?.count).toBe(2);
		expect(oneWound?.probability).toBeCloseTo(33.33, 1);

		const twoWounds = results.probabilityDistribution.find((p) => p.wounds === 2);
		expect(twoWounds?.count).toBe(3);
		expect(twoWounds?.probability).toBe(50);

		const threeWounds = results.probabilityDistribution.find((p) => p.wounds === 3);
	...
	it("should calculate cumulative probability correctly", () => {
		...
		const sorted = results.probabilityDistribution.sort((a, b) => a.wounds - b.wounds);
