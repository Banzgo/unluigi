 ...
 */
export function getProbabilityAtLeast(distribution: ProbabilityPoint[], targetWounds: number): number {
 ...
 * @returns Probability (0-100)
 */
export function getProbabilityExact(distribution: ProbabilityPoint[], targetWounds: number): number {
	const point = distribution.find((p) => p.wounds === targetWounds);
